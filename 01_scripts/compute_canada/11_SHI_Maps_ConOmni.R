...
for(year in v_time_steps_all){
  ...
  l_sp_Omnisc_files2 <- l_sp_Omnisc_files |> setNames(v_sp) |> compact()
  ...
  for(i in 2:n){
    ...
    print(names(l_sp_Omnisc_files2[[i]]))
  ...
  print(r_Omnisc_sum)
  writeRaster(r_Omnisc_sum,file.path(outputFolder,paste0("sum_Omnisc",year,".tif")),overwrite=T, gdal=c("COMPRESS=DEFLATE"), filetype="COG")
  #-------------------------------------------------------------------------------
