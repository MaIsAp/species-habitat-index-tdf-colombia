...
packages <- c("unix","dplyr","terra","geodata","purrr","readr","ggplot2")
...
# s_omniscape_output <- mask(s_omniscape_output0,s_habitat>=habitat_cutoff,maskvalues=1,inverse=T)
...
# map2(as.list(s_omniscape_output),v_time_steps_all,~writeRaster(.x,paste0(outputFolder,"/",sp,"_masked_con_",.y,".tif"),overwrite=T, gdal=c("COMPRESS=DEFLATE"), filetype="COG"))
...
df_conn_data <- tibble(Year= v_time_steps_all, 
  ...
  mutate(ref_curr=first(current),diff=ref_curr-current, 
          Values=current*100/ref_curr,sci_name=sp,Score="CS",Version="HypBase_year_Omniscape") |>
  relocate(Score,.after=Values) |>  relocate(sci_name,.before=Year)

print(df_conn_data )

write_csv(df_conn_data,paste0(outputFolder,"/",sp,"_df_conn_scoreOmni_HypBase.csv"))
...
img_SHS_frag <- ggplot(df_SHS_GISfrag,aes(x=Year,y=Values,col=Score, linetype=Score))+
  geom_line(linewidth=1,show.legend=F)+ scale_linetype_manual(values=c(rep("dashed",2),"solid")) +
  geom_point(size=2)+scale_colour_brewer(palette="Dark2")+scale_y_continuous(breaks=seq(0,110,10))+ coord_cartesian(ylim=c(0,110),xlim=c(2000,2020))+
  ylab("Connectivity Score (CS), Habitat Score (HS), \n Species Habitat Score (SHS)")+theme_bw()+theme(legend.position="bottom")+
  ggtitle("GIS frag - Hypothetical Base year") + scale_x_continuous(labels=v_time_steps,breaks=v_time_steps)# Ini replace the time in v_time_steps_all

ggsave(paste0(outputFolder,"/",sp,"_img_SHS_GISfrag_ts.png"),img_SHS_frag,width=8,height=5)

print("---------TS for GIS frag created----------")

# create table with SHS measured with Omniscape
df_AS <- df_SHS_GISfrag |> filter(Score=="AS") |> mutate(Version="HypBase_year_Omniscape")
...
write_csv(df_SHS_Omnis,paste0(outputFolder,"/",sp,"_df_SHS_HypBase_year_Omniscape.csv"))
# df_SHS_Omnis <- read_csv(paste0(outputFolder,"/",sp,"_df_SHS_HypBase_year_Omniscape.csv"))

print(df_SHS_Omnis)

img_SHS <- ggplot(df_SHS_Omnis,aes(x=Year,y=Values,col=Score, linetype=Score))+
  geom_line(linewidth=1,show.legend=F)+ scale_linetype_manual(values=c(rep("dashed",2),"solid")) +
  geom_point(size=2)+scale_colour_brewer(palette="Dark2")+scale_y_continuous(breaks=seq(0,110,10))+ coord_cartesian(ylim=c(0,110),xlim=c(2000,2020))+
  ylab("Connectivity Score (CS), Habitat Score (HS), \n Species Habitat Score (SHS)")+theme_bw()+theme(legend.position="bottom")+
  ggtitle("Omniscape - Hypothetical Base year") + scale_x_continuous(labels=v_time_steps,breaks=v_time_steps)# Ini replace the time in v_time_steps_all

ggsave(paste0(outputFolder,"/",sp,"_img_SHS_Omni_HypBase_ts.png"),img_SHS,width=8,height=5)
...
#-------------------------------------------------------------------------------
# # mask to area of habitat
# s_omniscape_output2 <- mask(s_omniscape_output0[[-1]],s_habitat[[2]]>=habitat_cutoff,maskvalues=1,inverse=T)
# 
# global_stats_prop_curr2 <- global(s_omniscape_output2,c("min","mean","max","median","sd"),na.rm=T)
# print(global_stats_prop_curr2) 

df_conn_data_t0 <- tibble(Year= v_time_steps, 
                          min_curr= global_stats_prop_curr$min[-1],
                          sum_curr= global_stats_prop_curr$sum[-1],
                          current = global_stats_prop_curr$mean[-1],
                          max_curr= global_stats_prop_curr$max[-1],
                          sd_curr= global_stats_prop_curr$sd[-1]) |>
  mutate(ref_curr=first(current),diff=ref_curr-current, 
          Values=current*100/ref_curr,sci_name=sp,Score="CS",Version="t0Base_year_Omniscape") |> 
  relocate(Score,.after=Values) |>  relocate(sci_name,.before=Year)

print(df_conn_data_t0)

write_csv(df_conn_data_t0,paste0(outputFolder,"/",sp,"_df_conn_scoreOmni_t0Base.csv"))
...
img_SHS_frag2 <- ggplot(df_SHS_GISfrag2,aes(x=Year,y=Values,col=Score, linetype=Score))+
  geom_line(linewidth=1,show.legend=F)+ scale_linetype_manual(values=c(rep("dashed",2),"solid")) +
  geom_point(size=2)+scale_colour_brewer(palette="Dark2")+ coord_cartesian(ylim=c(0,100),xlim=c(2000,2020))+
  ylab("Connectivity Score (CS), Habitat Score (HS), \n Species Habitat Score (SHS)")+theme_bw()+theme(legend.position="bottom")+ 
  ggtitle("GIS frag - t0 base year = 2000") + scale_x_continuous(labels=v_time_steps,breaks=v_time_steps)# Ini replace the time in v_time_steps_all

ggsave(paste0(outputFolder,"/",sp,"_img_SHS_GISfrag2_ts.png"),img_SHS_frag2,width=8,height=5)

print("---------TS for GIS frag 2 created----------")

# create table with SHS measured with Omniscape
df_AS2 <- df_SHS_GISfrag2 |> filter(Score=="AS") |> mutate(Version="t0Base_year_Omniscape")
...
write_csv(df_SHS_Omnis2,paste0(outputFolder,"/",sp,"_df_SHS_t0Base_year_Omniscape.csv"))

print(df_SHS_Omnis2)

img_SHS2 <- ggplot(df_SHS_Omnis2,aes(x=Year,y=Values,col=Score, linetype=Score))+
  geom_line(linewidth=1,show.legend=F)+ scale_linetype_manual(values=c(rep("dashed",2),"solid")) +
  geom_point(size=2)+scale_colour_brewer(palette="Dark2")+ coord_cartesian(ylim=c(0,100),xlim=c(2000,2020))+
  ylab("Connectivity Score (CS), Habitat Score (HS), \n Species Habitat Score (SHS)")+theme_bw()+theme(legend.position="bottom")+ 
  ggtitle("Omniscape - t0 base year = 2000") + scale_x_continuous(labels=v_time_steps,breaks=v_time_steps)

ggsave(paste0(outputFolder,"/",sp,"_img_SHS_Omni_t0Base_ts.png"),img_SHS2,width=8,height=5)
...
write_csv(df_SHS_all,paste0(outputFolder,"/",sp,"_df_SHS.csv"))
