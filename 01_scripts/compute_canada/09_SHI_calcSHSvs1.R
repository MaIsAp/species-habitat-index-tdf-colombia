...
s_omniscape_output <- mask(s_omniscape_output0,terra::vect(sf_area_lim1_srs))
...
map2(as.list(s_omniscape_output),v_time_steps_all,~writeRaster(.x,paste0(outputFolder,"/",sp,"_masked_con_tdf_",.y,".tif"),overwrite=T, gdal=c("COMPRESS=DEFLATE"), filetype="COG"))
...
df_conn_data <- tibble(Year= v_time_steps_all, 
  ...
  mutate(ref_curr=first(current),diff=ref_curr-current, 
         Values=current*100/ref_curr,sci_name=sp,Score="CS",Version="HypBase_year_Omniscape_tdf") |>
  relocate(Score,.after=Values) |>  relocate(sci_name,.before=Year)

print(df_conn_data )

write_csv(df_conn_data,paste0(outputFolder,"/",sp,"_df_conn_scoreOmni_tdf_HypBase.csv"))
...
df_AS <- df_SHS_GISfrag |> filter(Score=="AS") |> mutate(Version="HypBase_year_Omniscape_tdf")
...
write_csv(df_SHS_Omnis,paste0(outputFolder,"/",sp,"_df_SHS_HypBase_year_Omniscape_tdf.csv"))
...
df_conn_data_t0 <- tibble(Year= v_time_steps, 
  ...
  mutate(ref_curr=first(current),diff=ref_curr-current, 
         Values=current*100/ref_curr,sci_name=sp,Score="CS",Version="t0Base_year_Omniscape_tdf") |> 
  relocate(Score,.after=Values) |>  relocate(sci_name,.before=Year)

print(df_conn_data_t0)

write_csv(df_conn_data_t0,paste0(outputFolder,"/",sp,"_df_conn_scoreOmni_tdf_t0Base.csv"))
...
# create table with SHS measured with Omniscape
df_AS2 <- df_SHS_GISfrag2 |> filter(Score=="AS") |> mutate(Version="t0Base_year_Omniscape_tdf")
...
write_csv(df_SHS_Omnis2,paste0(outputFolder,"/",sp,"_df_SHS_t0Base_year_Omniscape_tdf.csv"))
...
write_csv(df_SHS_all,paste0(outputFolder,"/",sp,"_df_SHS_tdf.csv"))
